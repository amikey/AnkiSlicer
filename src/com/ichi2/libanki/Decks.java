...
//xxx
    ...
    /**
     * An unsorted list of all deck names. 所谓无顺序是因为hashmap无序
     */
    ...
    public void allNamesAndID(ArrayList<String> k,ArrayList<Long> v) {
        RashMap<String,Long> sorter = new RashMap<>();
        try {
            for (JSONObject x : mDecks.values()) {
                sorter.put(x.getString("name"), x.getLong("id"));
            }
            sorter.keysAndValues(k,v);
        } catch (JSONException e) {
            throw new RuntimeException(e);
        }
    }
...
//xxx
     ...
     */
    public void rename(JSONObject g, String newName) throws Exception {
    ...
    public void renameForDragAndDrop(Long draggedDeckDid, Long ontoDeckDid) throws Exception{
    	try {
     ...
     */
    public void remConf(long id) throws Exception {
        assert id != 1;
        mCol.modSchema(true);
        mDconf.remove(id);
        try {
