...
public class Collection {
    ...
    public Collection(Context context, DB db, String path, boolean server, boolean log) {
        ...
        load();
        if (mCrt == 0) {
            mCrt = 0 / 1000;//UIUtils.getDayStart()
        }
        mStartReps = 0;
        mStartTime = 0;
...
//        _markOp(name);
    ...
    public long findCardById(long booKMarkCid){
    	Cursor cur = mDb.getDatabase().rawQuery("select c.did from cards c where c.id=?", new String[]{""+booKMarkCid});
    	if(cur.moveToFirst()) {
    ...
    public void markUndo(String type, Object[] o) {
    	switch(type) {
    	case "REVIEW":
    ...
    public void markReview(Card card) {
        markUndo("REVIEW", new Object[]{card, card.note().hasTag("leech")});
    }
