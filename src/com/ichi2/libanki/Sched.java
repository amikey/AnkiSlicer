...
public class Sched {
    ...
    public CharSequence finishedMsg(Context context) {
        ...
        sb.append("studyoptions_congrats_finished");
    ...
    public String _nextDueMsg(Context context) {
        ...
        if (revDue()) {
            sb.append("\n\n");
            sb.append("studyoptions_congrats_more_rev");
        }
        if (newDue()) {
            sb.append("\n\n");
            sb.append("studyoptions_congrats_more_new");
        }
        if (haveBuried()) {
            String now;
            if (mHaveCustomStudy) {
                now = " " + "sched_unbury_action";
            } else {
                now = "";
            }
            sb.append("\n\n");
            sb.append("" +"sched_has_buried" + now);
        }
        try {
            if (mHaveCustomStudy && mCol.getDecks().current().getInt("dyn") == 0) {
                sb.append("\n\n");
                sb.append("studyoptions_congrats_custom");
            }
        } catch (JSONException e) {
            throw new RuntimeException(e);
        }
